...
Vagrant.configure('2') do |config|
  ...
  %w(vmware_fusion vmware_workstation).each do |vmware|
    config.vm.provider vmware do |_v, override|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = '%s-%02d' % [$instance_name_prefix, i] do |config|
      ...
      if $enable_serial_logging
        ...
        %w(vmware_fusion vmware_workstation).each do |vmware|
          config.vm.provider vmware do |v, _override|
      ...
      # config.vm.synced_folder "/Users/joycechan/work/damper", "/damper", id: "core", :nfs => true, :mount_options => ['nolock,vers=3,udp']
      ...
      if File.exist?(CLOUD_CONFIG_PATH)
        ...
        config.vm.provision :shell, inline: 'curl -L https://github.com/docker/compose/releases/download/1.11.2/docker-compose-`uname -s`-`uname -m` > /usr/local/bin/docker-compose && chmod 755 /usr/local/bin/docker-compose'
      end
    end
  end
end
